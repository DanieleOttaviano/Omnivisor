...
usage() {
    cat <<EOF
$(basename "$0") - Apply or remove ATF patches
...
apply_patches_in_dir() {
    local patch_dir="$1"
    local full_dir="$custom_atf_patch_dir/$patch_dir"
    ...
    for patch_file in $patches; do
        ...
        if ! patch $REMOVE -p1 -d "$atf_dir" < "$patch_file"; then
...
# Single patch
if [[ -n "$PATCH" ]]; then
    local_patch="$custom_atf_patch_dir/$PATCH"
    if [[ ! -f "$local_patch" ]]; then
        error "Patch not found: $PATCH"
        info "Available patches:"
        ls "$custom_atf_patch_dir"
        exit 1
    fi

    info "${OPERATION^} single patch: $PATCH"
    if patch $REMOVE -p1 -d "$atf_dir" < "$local_patch"; then
